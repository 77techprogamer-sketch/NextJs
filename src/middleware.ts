...
export async function middleware(request: NextRequest, event: NextFetchEvent) {
    ...
    if (effectiveParts[1] === 'locations') {
        ...
        // Also handle /locations/[neighborhood-city] -> /locations/[state]/[city]
        if (effectiveParts.length === 3) {
            const slug = effectiveParts[2];
            let citySlug = slug;
            let state = CITY_TO_STATE.get(slug);

            if (!state && slug.includes('-')) {
                const parts = slug.split('-');
                const potentialCity = parts[0];
                const potentialState = CITY_TO_STATE.get(potentialCity);
                if (potentialState) {
            ...
            if (state && !ALL_STATES.has(slug)) {
                targetPath = `/locations/${state}/${citySlug}`;
            }
        }
        // Handle /locations/[city]/[service] -> /locations/[state]/[city]/[service]
            ...
            let citySlug = slug1;
            let state = CITY_TO_STATE.get(slug1);

            if (!state && slug1.includes('-')) {
            ...
            if (state && isServiceForSlug2) {
                targetPath = `/locations/${state}/${citySlug}/${slug2}`;
            }
        }

        // Priority Check: If it's a valid canonical city/service page, ensure it's a priority city
        const canonicalParts = (targetPath || effectivePathname).split('/');
        let cityToCheck = null;
        
        if (canonicalParts.length === 4 && !ALL_STATES.has(canonicalParts[3])) {
            cityToCheck = canonicalParts[3]; // /locations/[state]/[city]
        } else if (canonicalParts.length === 5) {
            cityToCheck = canonicalParts[3]; // /locations/[state]/[city]/[service]
        }

        if (cityToCheck && !PRIORITY_CITY_SLUGS.has(cityToCheck)) {
            return NextResponse.redirect(new URL(localeInPath ? `/${localeInPath}` : '/', request.url), 301);
        }
