...
const VisitorCounter = () => {
  ...
  const containerClasses = cn(
    "fixed text-muted-foreground bg-background/80 backdrop-blur-sm rounded-lg flex flex-col items-start shadow-lg border animate-in fade-in-0 duration-700",
    isMobile ? "bottom-2 left-2 p-2 space-y-1 text-xs" : "bottom-4 left-4 p-3 space-y-2 text-xs"
  );
    ...
    <div className={containerClasses}>
      <div className="flex items-center" title="Total visits">
        ...
        <div className="flex flex-col items-start">
          <span>{t("visitor_total_visits", { count: totalVisits, defaultValue: "{{count}} total visits" })}</span>
          {lastVisit && <span className="text-muted-foreground/70">{t("visitor_last_visit", { time: lastVisit, defaultValue: "Last visit {{time}}" })}</span>}
        </div>
      </div>
      <div className="flex items-center" title="Visitors currently online">
        <Users className="h-4 w-4 mr-2 text-green-500" />
        <span>{t("visitor_currently_online", { count: onlineCount, defaultValue: "{{count}} currently online" })}</span>
      </div>
    </div>
  );
};
