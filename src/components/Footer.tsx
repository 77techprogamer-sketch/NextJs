...
import { ShieldCheck, Award, Calculator, Target, FileSearch, Activity } from 'lucide-react';
const Footer = () => {
    ...
    <footer className="w-full bg-muted text-muted-foreground border-t">
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 mb-8">
          ...
          <div>
            <h4 className="font-bold text-foreground mb-4 uppercase tracking-wider text-xs">{t("tools", "Tools")}</h4>
            <ul className="space-y-2 text-sm">
              <li><Link href="/tools/human-life-value-calculator" className="hover:text-primary transition-colors flex items-center gap-2"><Calculator className="w-4 h-4" />{t("hlv_calc_title", "HLV Calculator")}</Link></li>
              <li><Link href="/tools/risk-scorecard" className="hover:text-primary transition-colors flex items-center gap-2"><Target className="w-4 h-4" />{t("risk_scorecard_title", "Risk Scorecard")}</Link></li>
              <li><Link href="/tools/policy-recovery" className="hover:text-primary transition-colors flex items-center gap-2"><FileSearch className="w-4 h-4" />{t("lost_policy_recovery_tool", "Policy Recovery")}</Link></li>
              <li><Link href="/tools/term-insurance-calculator" className="hover:text-primary transition-colors flex items-center gap-2"><Activity className="w-4 h-4" />{t("term_calculator_title", "Term Calculator")}</Link></li>
              <li><Link href="/tools" className="hover:text-primary transition-colors text-primary font-medium">{t("view_all_tools", "View All Tools")}</Link></li>
            </ul>
          </div>
          <div>
          ...
          <div className="text-center sm:text-left">
            ...
            <p className="text-sm mb-2 font-semibold text-primary flex items-center gap-2 justify-center sm:justify-start">
              ...
              IRDAI Reg No: 014911D
            </p>
            <p className="text-xs font-medium uppercase tracking-tighter opacity-70 mt-4">Founded by Hari Kotian with 25+ years of insurance experience</p>
          </div>
        </div>
