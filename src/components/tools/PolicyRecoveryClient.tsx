...
export default function PolicyRecoveryClient() {
    const { t } = useTranslation();

    const handleHandoff = async (data: any) => {
        // Save the quiz data as a lead
        try {
            await leadService.submitLead({
                fullName: 'Policy Recovery Quiz',
                email: '',
                phone: '',
                insurance_type: 'policy_recovery',
                details: {
                    source: 'policy_recovery_quiz',
                    quiz_data: data?.formData || {},
                    insuranceType: data?.insuranceType || 'policy_recovery'
                }
            }, 'PolicyRecoveryQuiz');
        } catch (err) {
            console.error('Failed to save policy recovery lead:', err);
        }
        
        // Trigger the global quote form handler
