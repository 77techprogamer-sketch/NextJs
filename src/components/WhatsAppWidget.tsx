            ...
            <AnimatePresence>
                {isVisible && (
                    <div className="flex flex-col items-start gap-3 pointer-events-auto">
                        ...
                        <AnimatePresence>
                            {showTooltip && (
                                <motion.div
                                    ...
                                    className="hidden sm:block relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 p-3 pr-8 rounded-2xl rounded-bl-none shadow_xl max-w-[200px]"
                                >
                                    <button
                        ...
                        >
                            {/* Pulsing ring - Hide on mobile to reduce noise */}
                            <div className="hidden sm:block absolute inset-0 bg-[#25D366] rounded-full animate-ping opacity-20" />

                            <div className="relative w-12 h-12 sm:w-14 sm:h-14 bg-[#25D366] text-white rounded-full shadow-[0_10px_30px_-5px_rgba(37,211,102,0.6)] flex items-center justify-center transition-all">
                                <MessageCircle className="w-6 h-6 sm:w-8 sm:h-8 fill-current" />
                            </div>

                            {/* Online Badge */}
                            <div className="absolute top-0 right-0 w-3 h-3 sm:w-4 sm:h-4 bg-white dark:bg-slate-900 rounded-full flex items-center justify-center border-2 border-[#25D366]">
                                <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 bg-[#25D366] rounded-full animate-pulse" />
                            </div>
                        </motion.a>
                    </div>
                )}
