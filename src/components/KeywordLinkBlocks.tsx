...
// Only include city slugs that have valid hub pages (excludes neighbourhood sub-keys)
...
export const CityLinksForService: React.FC<CityLinksForServiceProps> = ({ serviceSlug, serviceTitle }) => {
    // Only link to cities with valid /locations/{state}/{city}/{service} pages
    const locations = Object.keys(cityData).filter(key => validCitySlugs.has(key));
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 text-sm">
                {locations
                    ...
                    .map((cityKey) => (
                        <Link
                            key={cityKey}
                            href={`/locations/${slugify(cityData[cityKey].state)}/${cityKey}/${serviceSlug}`}
...
export const ServiceLinksForCity: React.FC<ServiceLinksForCityProps> = ({ citySlug, cityName, cityState }) => {
    return (
        <div className="mt-12 pt-8 border-t border-slate-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                {services.map((serviceSlug) => (
                    <Link
                        key={serviceSlug}
                        href={`/locations/${slugify(cityState)}/${citySlug}/${serviceSlug}`}
                    ...
                    >
                        <span className="font-semibold text-slate-700 group-hover:text-primary transition-colors">
                            Best {serviceLabels[serviceSlug]} Insurance Support in {cityName}
                        </span>
