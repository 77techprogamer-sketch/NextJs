...
// Only include city slugs that have valid top-level hub pages in INDIAN_LOCATIONS
const validCitySlugs = new Set(INDIAN_CITIES);

export default function DynamicKeywordLinks() {
    // Filter to only cities with valid hub pages (exclude neighbourhood sub-keys like bangalore-indiranagar)
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        {featuredCities.map(city => (
                            <Link
                                key={city}
                                href={`/locations/${slugify(cityData[city]?.state || 'in')}/${city}`}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
                            {featuredServices.slice(4, 8).map((service, index) => {
                                ...
                                return (
                                    <Link
                                        key={`${service}-${city}`}
                                        href={`/locations/${slugify(cityData[city]?.state || 'in')}/${cityData[city]?.slug || city}/${service}`}
