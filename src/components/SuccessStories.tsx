...
export default function SuccessStories({ filterCategory, filterCity, limit = 2 }: SuccessStoriesProps) {
    let filteredStories = successStories;
    
    if (filterCategory) {
        filteredStories = filteredStories.filter(s => s.category.toLowerCase().includes(filterCategory.toLowerCase()));
    }
    
    if (filterCity) {
        // Simple fuzzy match for city
        const cityStories = filteredStories.filter(s => s.location.toLowerCase().includes(filterCity.toLowerCase()));
        // If we have local stories, show them first. If not, show general ones.
        if (cityStories.length > 0) {
            filteredStories = cityStories;
        }
    }
                        ...
                        <Card key={story.id} className="overflow-hidden border-none shadow-xl hover:shadow-2xl transition-all duration-300">
                            <CardHeader className="bg-white border-b border-slate-100 pb-4">
                                ...
                                <CardTitle className="text-xl md:text-2xl text-slate-900 leading-tight">
                                    <span className="text-primary block text-sm uppercase tracking-widest mb-2 font-black">Case Study</span>
                                    {story.title}
                                </CardTitle>
                            </CardHeader>
