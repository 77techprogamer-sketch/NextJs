        ...
        <div className="flex flex-col md:flex-row gap-12">
            ...
            <div className="flex-1 max-w-3xl">
                <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
                    ...
                    <span>/</span>
                    <Link href={`/locations/${slugify(city.state)}/${city.slug}`} className="hover:text-primary">{city.name}</Link>
                    <span>/</span>
                    ...
                    <div>
                        ...
                        <ul className="space-y-2 text-sm">
                            {services.filter(s => s !== serviceSlug).slice(0, 5).map(s => (
                                <li key={s}>
                                    <Link href={`/locations/${slugify(city.state)}/${city.slug}/${s}`} className="text-slate-600 hover:text-primary hover:underline">
                    ...
                    <div>
                        ...
                        <ul className="space-y-2 text-sm">
                            {city.nearbyCities && city.nearbyCities.length > 0 ? (
                                city.nearbyCities
                                    .filter((slug: string) => validCitySlugs.has(slug) && (allowedCities === null || allowedCities.includes(slug)))
                                    .map((slug: string) => {
                                        const nearby = allCityData[slug];
                                        return nearby ? (
                                            <li key={slug}>
                                                <Link href={`/locations/${slugify(nearby.state)}/${slug}/${serviceSlug}`} className="text-slate-600 hover:text-primary hover:underline">
