...
export default function SitemapIndexPage() {
    const locations = [...INDIAN_LOCATIONS].sort((a, b) => a.name.localeCompare(b.name))
    const staticRoutes = [
        { name: 'Home', path: '/' },
                    ...
                    <section>
                        ...
                        <div className="max-h-[500px] overflow-y-auto pr-4 scrollbar-thin scrollbar-thumb-slate-200">
                            <ul className="space-y-3">
                                {locations.map(city => (
                                    <li key={city.city}>
                                        <Link href={`/locations/${city.state}/${city.city}`} className="text-sm text-slate-600 hover:text-primary transition-colors font-medium">
                                            Advisor in {city.name}
                                        </Link>
                                        <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1">
                                            {services.map(service => (
                                                <Link
                                                    key={`${city.city}-${service}`}
                                                    href={`/locations/${city.state}/${city.city}/${service}`}
