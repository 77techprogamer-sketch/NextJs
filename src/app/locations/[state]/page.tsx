...
export default async function StateHubPage({ params }: Props) {
    ...
    if (cityAsState) {
        // Redirect to the canonical 3-segment URL
        redirect(`/locations/${cityAsState.state}/${cityAsState.city}/life-insurance`);
    }
                ...
                <div className="space-y-8">
                    <div className="bg-slate-900 text-white p-8 rounded-3xl shadow-xl">
                        ...
                        <ul className="space-y-3 text-sm text-slate-300">
                            <li className="flex items-center gap-2 font-medium"><span className="text-primary text-xl leading-none">â€¢</span> {t('cashless_hospitalization', 'Cashless Hospitalization Support')}</li>
                            <li className="flex items-center gap-2 font-medium"><span className="text-primary text-xl leading-none">â€¢</span> {t('dead_claim_specialists', 'LIC Dead-Claim Specialists')}</li>
                            <li className="flex items-center gap-2 font-medium"><span className="text-primary text-xl leading-none">â€¢</span> {t('motor_claims', 'Motor Accidental Claims')}</li>
                            <li className="flex items-center gap-2 font-medium"><span className="text-primary text-xl leading-none">â€¢</span> {t('doorstep_pickup', 'Doorstep Document Pickup')}</li>
                        </ul>
                    </div>
                </div>
            </div>
