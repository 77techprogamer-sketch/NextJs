...
import { ChevronRight, ArrowRight, MapPin, Grid, ShieldCheck, Clock, Award, Landmark, Target, Zap, Scale } from 'lucide-react'
...
export async function generateStaticParams() {
    ...
    // Ensure we cover ALL priority cities in indianCities.ts for EACH service
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const serviceOverride = override?.serviceSpecific?.[params.service];
    ...
    const finalSummary = serviceOverride?.summary || override?.summary;
    const description = finalSummary && lang === 'en'
        ? `${finalSummary.substring(0, 150)}... ${t('location_meta_desc_expert', { service: serviceLabel.toLowerCase(), city: localizedCity, phone: contactConfig.phone })}`
        ...
        <React.Fragment>
            ...
            <script
                ...
                dangerouslySetInnerHTML={{ __html: JSON.stringify({
                    ...
                    itemListElement: [
                        ...
                        { '@type': 'ListItem', position: 4, name: `${serviceLabel} in ${localizedCity}`, item: `https://insurancesupport.online/locations/${params.state}/${params.city}/${params.service}` },
                    ]
                }) }}
            />
            <main className="container mx-auto px-4 py-8 md:py-16">
                <div className="max-w-4xl mx-auto">
                    ...
                    {displayFacts.length > 0 && (
                        <div className="mb-12 p-8 bg-blue-50/50 dark:bg-blue-900/10 rounded-3xl border border-blue-100 dark:border-blue-800">
                            <h2 className="text-xl font-bold mb-6 flex items-center gap-2 text-blue-800 dark:text-blue-300">
                                ...
                                {t('location_page.why_best_in_city', { service: serviceLabel, city: localizedCity })}
                            </h2>
                            ...
                            <p className="mt-8 text-sm italic text-slate-500 dark:text-slate-400 border-t border-blue-100 dark:border-blue-800 pt-4">
                                💡 <strong>{t('local_insight', 'Local Insight')}:</strong> {displayLocalContext}
                            </p>
                        </div>
                    )}
                    ...
                    {/* Cluster Strategy: Upward link to Pillar Page */}
                    <div className="mb-16 p-8 bg-slate-900 rounded-[40px] text-white flex flex-col md:flex-row items-center gap-8 shadow-2xl relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-8 opacity-10">
                            <Zap className="w-32 h-32 text-primary" />
                        </div>
                        <div className="flex-shrink-0 w-20 h-20 bg-primary/20 rounded-3xl flex items-center justify-center text-primary">
                    ...
                    <div className="mt-16 pt-16 border-t border-slate-100 dark:border-slate-800">
                        <SuccessStories filterCategory={serviceCategory} filterCity={localizedCity} />
                    </div>
