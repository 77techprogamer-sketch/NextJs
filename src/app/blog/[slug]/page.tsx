...
export default async function BlogPostPage({ params }: BlogPostProps) {
    ...
    // Get translated content if available, otherwise fall back to default content
    const translatedContent = post.translations?.[lang] || post.content;
    const translatedTitle = post.translations?.[lang]?.title || post.title;
    const translatedSummary = post.translations?.[lang]?.summary || post.summary;
                        ...
                        <header className="p-8 md:p-12 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30">
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-4 text-slate-500 dark:text-slate-400 text-sm">
                                <div className="flex items-center gap-2 font-medium">
                                    ...
                                    <Link href="/about" rel="author" className="hover:text-primary transition-colors">
                                        Hari Kotian
                                    </Link>
                                </div>
